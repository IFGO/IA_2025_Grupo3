...
from utils.statistical import run_statistical
...
def args_parser() -> None:
    ...
    parser.add_argument('--model', type=str, default="none", help='Tipo de modelo: MLPRegressor')
    ...
    parser.add_argument('--statistical', type=bool, default=False, help='Caso esteja habilitado, ira analisar 10 moedas presentes no diretorio data,' \
    ' gerando gráficos e csv de dados.')
    ...
    try:
        ...
        if args.model not in ['mlp', 'linear', 'poly']:
            ...
            for model in ['mlp', 'linear', 'poly']:
                ...
                data[model] = {'predict': predictions_df, 'backtest': backtest_results}
        ...
        if args.statistical:
            logger.info("Análise estatística habilitada para 10 moedas, executando...")
            run_statistical()
...
def execute(args, featured_data):
    ...
    return predictions_df, backtest_results
